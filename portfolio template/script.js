...
//CLOSE OPEN MODAL/IMAGES EFFECT
const closeModal = () => {
...
//ACTIVE NAVIGATION UNDERLINE EFFECT
...
const observer = new IntersectionObserver(activeNav, {root: null,threshold: 0.12,});
const sectionAppear = new IntersectionObserver(loadSection, {root: null,threshold: 0.15,});
...
images.forEach((image) => {
  image.addEventListener("click", () => {
    ...
    imgsrc.setAttribute(
      "src",
      window.getComputedStyle(image).backgroundImage.slice(27, -2)
    );
  });
});
...
sliderFunction(slides, btnright, btnleft, dotcontainer, "a");
